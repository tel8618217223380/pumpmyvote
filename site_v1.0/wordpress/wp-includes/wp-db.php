...
class wpdb {
	...
	/**
	 * Pumpmyvote comment_arti_eksi table
	 *
	 * @since 3.0.1
	 * @access public
	 * @var string
...
<p>Veritabanı sunucusuna bağlanabiliyoruz (ki bu da kullanıcı adı ve şifrenizin doğru olduğu anlamına geliyor) fakat <code>%1$s</code> veritabanını seçemiyoruz.</p>
<ul>
<li>Var olduğundan emin misiniz?</li>
<li><code>%2$s</code> kullanıcısının <code>%1$s</code> veritabanı için gerekli yetkileri mevcut mu?</li>
<li>Bazı sistemlerde sistem, veritabanı isminin önünde kullanıcı adınızı otomatik yerleştirir, <code>kullanici_%1$s</code> vb. Problem bu olabilir mi?</li>
</ul>
<p>Eğer nasıl veritabanı kuracağınızı/oluşturacağınızı bilmiyorsanız <strong>yayıncı firmanız ile irtibat kurun</strong>. Hiç bir yerden çözüm bulamazsanız <a href="http://www.wordpress-tr.com/forum/">WordPress Türkiye Destek Forumları</a> her zaman sizin için hazır.</p>'/*/WP_I18N_DB_SELECT_DB*/, $db, $this->dbuser ), 'db_select_fail' );
			return;
		}
	}
